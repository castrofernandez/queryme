...
var validateOptions = function validateOptions(options) {
  getWrongTypeOptions(options).forEach(function (diff) {
    console.log("The option \"".concat(diff.key, "\" is expected to be \"").concat(diff.expected, "\"") + " but received as \"".concat(diff.actual, "\"."));
  });
  getInvalidOptions(options).forEach(function (diff) {
    console.log("The option \"".concat(diff.key, "\" is not valid."));
  });
};
